...
import { Get, JsonController, QueryParam, Param, Session, Ctx, Post, BodyParam, HeaderParams, Delete} from 'routing-controllers'
...
export class TaskController {
  ...
  async createTask(@HeaderParams() param:any,@Ctx() ctx:any): Promise<any> {
    ...
    if(user){
        ...
        try {
          const submitResult = await SlurmService.submitjob(user, taskname, filename, type, amount, tasks);
