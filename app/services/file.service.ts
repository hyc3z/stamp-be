...
export class FileService {
  ...
  static async mkdir(username: string): Promise<boolean> {
    ...
    fsConfig.subPaths.forEach(subPath => {
      fs.mkdir(
        path.join(fsConfig.mountPath, username, subPath),
        { recursive: true },
        err => {
          if (err) {
            result = false
  ...
  static async saveFile(username: string, subpath: string, file: any): Promise<any> {
    ...
    fs.writeFile(userpath, file.buffer, err => {
      if (err) {
        console.log(err)
        result = false
      }
  ...
  static async convertObjectToChonky(
    file: directory_tree.DirectoryTree,
    globalList: any,
  ): Promise<any> {
    if (!file) return
    ...
    if (children) {
      children.forEach(async obj => {
        await this.convertObjectToChonky(obj, globalList)
      })
    }
  }
  ...
  static async convertObjectToDevExtreme(
    file: directory_tree.DirectoryTree,
  ): Promise<any> {
    if (!file) return
    const children = file.children
  ...
  static async convertObjectToChonkyWithoutFolders(
    file: directory_tree.DirectoryTree,
    globalList: any,
  ): Promise<any> {
    if (!file) return
    ...
    if (children) {
      children.forEach(async obj => {
        await this.convertObjectToChonkyWithoutFolders(obj, globalList)
      })
    }
  }
}
