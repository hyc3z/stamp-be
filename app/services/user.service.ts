...
export class UserService {
  ...
  static async deleteEnv(username: string, envId: number | string): Promise<any> {
    ...
    const existingEnv = await userEnvConnection.find({
      where: { envId: envId, userId: uid },
    })
  ...
  static async validate(username: string, password: string): Promise<boolean> {
    let allUsers = await getConnection()
      ...
      .find({
        where: { userName: username, userPwd: await EncriptService.hashed(password) },
      })
    // let allUsers = await suRepo.find();
