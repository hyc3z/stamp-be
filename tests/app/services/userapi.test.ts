...
test('User register.', async () => { 
    const response = await axios.get(`http://${config.host}:${config.port}/user/register?username=${testconfig.test_account}&password=${testconfig.test_pwd}`);
    expect(JSON.stringify(response.data["message"])).toMatch(/true|false/g)
})

test('User login.', async () => { 
    const response = await axios.get(`http://${config.host}:${config.port}/user/login?username=${testconfig.test_account}&password=${testconfig.test_pwd}`);
...
test('Use token to request, api exposed.', async () => {
    ...
    try{
        const response = await axios.get(`http://${config.host}:${config.port}/cluster/nodes`,{
            headers: {
